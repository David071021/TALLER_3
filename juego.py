...
def mover_robot(tablero, direccion):
    ...
    if tablero[fila_obj][columna_obj] == board.SPACE  :
        ...
        tablero[fila_obj][columna_obj] = board.PUNTO
...
def juego():
    nivel = input('Seleccione el nivel (1/2): ')
    if nivel == '1':
        tab = board.tablero
...
def manual(idioma):
    ...
    op=input('ingrese o para volver a menu: ')
    if op !='o':
        manual(idioma)
